...
# def get_parent_server_ip(local_server_id: uuid.UUID) -> Optional[str]:
#     """
#     Looks up the parent server's IP address for a given local server.
    
#     Args:
#         local_server_id: The UUID of the current LSC server.
        
#     Returns:
#         The IP address of the parent server, or None if not found.
#     """
#     try:
#         # Step 1: Find the local server's record
#         local_server = Server.objects.select_related('parent_server').get(server_id=local_server_id)
        
#         # Step 2: Get the parent server record via the parent_server ForeignKey
#         parent_server = local_server.parent_server
        
#         if parent_server and parent_server.ip_address:
#             # Step 3: Return the IP address
#             return parent_server.ip_address
#         else:
#             logger.warning(f"Parent server IP not found for local server {local_server_id}.")
#             return None
            
#     except Server.DoesNotExist:
#         logger.error(f"Local server with ID {local_server_id} not found in the database.")
#         return None
#     except Exception as e:
#         logger.error(f"An error occurred while getting parent IP: {e}")
#         return None
...
class SystemDetector:
    ...
    def get_system_uuid(self) -> Optional[uuid.UUID]:
        """
        Gets the system UUID using PowerShell on Windows.
        """
        if platform.system() == 'Windows':
            try:
                command = '(Get-WmiObject -Class Win32_ComputerSystemProduct).UUID'
    ...
    def get_complete_system_info(self) -> Dict:
        ...
        # --- Add the new system_uuid field ---
        system_uuid = self.get_system_uuid()
        if system_uuid:
            system_info['system_uuid'] = system_uuid
        # ---
