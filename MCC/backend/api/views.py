...
class MasterSyncAPIView(APIView):
    ...
    def process_sync_up(self, sync_items, user, source_device_id):
        ...
        with transaction.atomic():
            for item in sync_items:
                ...
                try:
                    if action == 'create':
                        if 'parent_admin_id' in data and data['parent_admin_id']:
                            data['parent_admin_id'] = Admin.objects.get(pk=data['parent_admin_id'])
    ...
    def process_sync_down(self, last_sync_timestamp, user):
        ...
        if not last_sync_timestamp:
            # First sync: send all relevant data for this user's license
        ...
        # Subsequent sync: find all changes since the last sync time
        for Model in MODEL_MAP.values():
